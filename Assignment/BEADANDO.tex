...
\end{align}
Here, $h_1$ is used to model the behavior in the low resistance state of the device, and $h_2$ captures its behavior in the high-resistance state.
The two electron transmission equations are weighted and mixed by the state
variable x which is set to take values between zero and one. In memristive devices, it is the rate of change of the state variable x that is explicitly determined (\ref{equa:2}), and is given in the Yakopcic memristor model by the product of the two composite functions $g(v)$ and f$(x)$ such that:
\begin{align}
     \dot{x} = g(v)f(x).
\end{align}
